INSERT (application_id, customer_name, loan_type, loan_amount, annual_income, credit_score, risk_score )
...
SELECT application_id, customer_name, loan_type, risk_score,
ROW_NUMBER () OVER (partition by loan_type ORDER BY risk_score DESC) AS row_num
FROM loan_applications;

SELECT application_id, customer_name, loan_amount, risk_score,
SUM(loan_amount) OVER (ORDER BY risk_score DESC) AS running_total
FROM loan_applications; 
